...
namespace tktk
{
	...
	┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
	*/
	template <class NodeType, class GetGroupFuncRunner>
	...
	// 衝突判定の組み合わせの種類を管理する配列のサイズを設定する
	template<class NodeType, class GetGroupFuncRunner>
	inline void RunFuncSimpleCollisionContainer<NodeType, GetGroupFuncRunner>::setCollisionGroupPairArrayCapacity(unsigned int capacity)
	{
	...
	// 衝突判定の組み合わせを追加する
	template<class NodeType, class GetGroupFuncRunner>
	inline void RunFuncSimpleCollisionContainer<NodeType, GetGroupFuncRunner>::addCollisionGroupPair(int first, int second)
	{
	...
	template<class NodeType, class GetGroupFuncRunner>
	template<class T>
	inline void RunFuncSimpleCollisionContainer<NodeType, GetGroupFuncRunner>::checkAndAdd(bool isStatic, ProcessingClassPtr newClass, T * ptr)
	{
		int collisionGroup = GetGroupFuncRunner::checkAndRun(newClass->castPtr<T>());
		AddClass addClass{ NodeType(newClass, ptr), collisionGroup, isStatic };
	...
	// 衝突判定を開始する
	template<class NodeType, class GetGroupFuncRunner>
	inline void RunFuncSimpleCollisionContainer<NodeType, GetGroupFuncRunner>::runFunc()
	{
	...
	// コンテナの要素を更新する
	template<class NodeType, class GetGroupFuncRunner>
	inline void RunFuncSimpleCollisionContainer<NodeType, GetGroupFuncRunner>::updateContainer()
	{
		moveContainerNode();
		removeDeleteClass();
	}
	...
	// コンテナの要素の関数を実行するかを設定する
	template<class NodeType, class GetGroupFuncRunner>
	inline void RunFuncSimpleCollisionContainer<NodeType, GetGroupFuncRunner>::setIsRunFunc(bool flag)
	{
		m_isRunFunc = flag;
	}
	...
	// ２つのグループで衝突判定を行う
	template<class NodeType, class GetGroupFuncRunner>
	inline void RunFuncSimpleCollisionContainer<NodeType, GetGroupFuncRunner>::collidePair(const std::pair<int, int>& groupPair, const std::unordered_map<int, std::vector<NodeType*>>& collideTempArrayMap)
	{
	...
	template<class NodeType, class GetGroupFuncRunner>
	inline void RunFuncSimpleCollisionContainer<NodeType, GetGroupFuncRunner>::collide(int secondGroup, NodeType* firstNodePtr, const std::unordered_map<int, std::vector<NodeType*>>& collideTempArrayMap)
	{
	...
	// addClassListの要素をrunFuncClassSortedArrayかrunFuncClassSortedListに移動する
	template<class NodeType, class GetGroupFuncRunner>
	inline void RunFuncSimpleCollisionContainer<NodeType, GetGroupFuncRunner>::moveContainerNode()
	{
	...
	// 削除されたクラスのポインタをコンテナから削除する
	template<class NodeType, class GetGroupFuncRunner>
	inline void RunFuncSimpleCollisionContainer<NodeType, GetGroupFuncRunner>::removeDeleteClass()
	{
		// 配列の要素削除
	...
	// 削除されたクラスのポインタを引数の配列から削除する
	template<class NodeType, class GetGroupFuncRunner>
	inline void RunFuncSimpleCollisionContainer<NodeType, GetGroupFuncRunner>::removeDeleteClassInNodeArray(std::vector<NodeType>* vectorPtr)
	{
