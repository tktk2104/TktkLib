...
namespace tktk
{
		...
		int rootSignatureIndex)
	{
		...
		graphicsPipeLineStateDesc.DepthStencilState.DepthEnable = initParam.m_useDepth;

		if (initParam.m_useDepth)
		{
			graphicsPipeLineStateDesc.DepthStencilState.DepthWriteMask = (initParam.m_writeDepth) ? D3D12_DEPTH_WRITE_MASK_ALL : D3D12_DEPTH_WRITE_MASK_ZERO;
			graphicsPipeLineStateDesc.DepthStencilState.DepthFunc = initParam.m_depthFunc;
