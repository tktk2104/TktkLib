#include "TktkDX12Wrapper/Graphics/DX3D12/DX3DResource/BufferResource/DepthStencilBuffer/DepthStencilBufferData.h"
...
namespace tktk
{
	...
	void DepthStencilBufferData::use(ID3D12GraphicsCommandList* commandList)
	{
	...
	void DepthStencilBufferData::unUse(ID3D12GraphicsCommandList* commandList)
	{
		if (!m_useAsShaderResource) return;
	...
	void DepthStencilBufferData::createShaderResourceView(ID3D12Device* device, D3D12_CPU_DESCRIPTOR_HANDLE heapHandle)
	{
	...
	const tktkMath::Vector2& DepthStencilBufferData::getDepthStencilSize() const
	{
